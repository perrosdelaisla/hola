...
- **Notificaciones push: [ntfy.sh](https://ntfy.sh/)** topic `perrosdelaisla-citas-2026` (constante `NTFY_TOPIC` en [victoria.js](victoria.js):57) — Charly lo recibe en el móvil. Hay **dos** disparadores:
  - **Cita confirmada** → `_notificarCarlos` (s12): manda un push interno (resumen + teléfono) y otro con el mensaje listo-para-WhatsApp. Además, al insertarse la fila en `citas`, se dispara el **aviso del panel admin vía trigger DB** (`app.trg_citas_insert_aviso`).
  - **Rescate de lead** → `_notificarRescate` (s9): cuando el lead dejó nombre+teléfono pero **aún no confirmó**. Solo push, **NUNCA toca la tabla `citas`** → no genera aviso de panel. Esa ausencia de aviso de panel es la señal que distingue un rescate de una cita confirmada.
...
### Núcleo
- **[victoria.js](victoria.js)** · orquestador principal. Máquina de estados `s0 → s12` (zona, nombre, datos del perro, problema, gravedad mordida, protocolo, agenda, slot, datos cliente, confirmación). **Ya NO hay paso de pago ni captura de comprobante ni widget de pago**: tras elegir slot, `s9` captura nombre/teléfono (+email si online) y `s12` confirma la cita directo (`_confirmarCitaSinPago` → `_procesarS12_Confirmacion`), naciendo con seña PENDIENTE. (Los procesadores `_procesarS10_Pago`/`_procesarS11_Captura` se conservan inertes solo por compatibilidad al restaurar sesiones antiguas.) Renderiza burbujas en el DOM, gestiona typing indicator, inserta la agenda como burbuja, persiste en Supabase, dispara las notificaciones a Carlos. Lee parámetros de URL: `?tema=` (preselección desde la app de Paseos), `?origen=` (canal de captación) y `?prueba=1` (sesión que no cuenta en stats).
...
> **`pagos.js` fue eliminado** (flujo sin pago en el chat). El bucket de Storage `pagos` sigue existiendo en Supabase pero ya no se sube nada. Quedan referencias inertes a limpiar: el CSS `.pago-*`/`.pay-*`/`.upload-*` en [styles.css](styles.css) (ver header de ese bloque).

### Capa Supabase
- **[supabase.js](supabase.js)** · wrapper `supa(path, method, body)` con headers de la anon key. Funciones para slots, bloqueos, citas, sesiones de tracking. Es el ÚNICO lugar donde construimos peticiones a `/rest/v1/`. (Excepción: `victoria.js` también hace `fetch` directo en sus flujos críticos — históricamente, no es ideal pero está estable; consultar antes de refactorizar).
...
### Otros
- **[index.html](index.html)** · estructura del splash + chat + input fijo. Importa `./victoria.js?v=66` (la query `?v=` se incrementa para invalidar caché en clientes).
...
### Slogan central
**"Tu perro merece ser feliz, hoy"** — aparece en el splash, en el cierre del chat tras confirmar la cita, y es el eje de toda la comunicación.
...
- **Seña de reserva: 45€** (50% de la primera clase presencial), por **Bizum**. **VictorIA NO la cobra dentro del chat**: la cita se confirma con seña PENDIENTE y es Carlos quien coordina el Bizum después (ver mensaje listo-para-WhatsApp en `_notificarCarlos`). El resto se paga en mano el día de la clase.
- **Dos números, dos roles distintos** — esto es crítico para el flujo de VictorIA:
  - **`622 922 173` — línea PÚBLICA del negocio.** Es la línea de captación y soporte general. **VictorIA la usa en TODO mensaje conversacional**: derivaciones, fallbacks (`_fallbackHumano`), errores técnicos, y la confirmación final de la cita. Es el único número que sale por boca de VictorIA en mensajes de chat.
  - **`653 591 301` — línea PRIVADA de Carlos (personal del adiestrador).** **VictorIA NUNCA lo expone en el chat.** El `653` solo aparece dentro del **mensaje listo-para-WhatsApp** que arma `_notificarCarlos` ([victoria.js](victoria.js):2506) y que Carlos copia/pega y envía él mismo al cliente para coordinar el Bizum de la seña. Es decir, el cliente recibe el `653` **por WhatsApp de Carlos**, no del bot. Carlos también usa este número para conversar manualmente con clientes activos tras la notificación ntfy — ese **traspaso de canal lo ejecuta él desde su móvil**, no VictorIA.
  - **Reglas de uso (no negociables):**
    - **VictorIA nunca pone el `653 591 301` en una burbuja de chat.** En todo mensaje conversacional (derivaciones, fallbacks, confirmación) usa el `622 922 173`. El `653` vive únicamente en el texto interno listo-para-WhatsApp de la notificación a Carlos.
    - **El traspaso de canal lo hace Carlos manualmente.** Cuando llega la notificación ntfy, él decide cuándo y cómo escribir al cliente desde su `653 591 301`. Es deliberado: VictorIA cierra la cita y el humano toma el relevo (incluida la coordinación de la seña).
- IBAN para transferencias: `ES27 0182 5319 7002 0055 6013` · Titular: Carlos Antonio Acevedo. (Ya no vive en el repo — estaba en el desaparecido `pagos.js`; conservado aquí como dato de negocio.)
...
- La anon key vive en claro en [victoria.js](victoria.js):56 y [supabase.js](supabase.js):7. Es la clave pública de Supabase — cualquier seguridad real debe venir de RLS en la base de datos, no de ocultar la key.
...
- **Resumen del flujo**: VictorIA habla → siempre `622`. El `653` solo viaja dentro de la notificación interna a Carlos, que lo reenvía él mismo. Carlos toma el relevo manualmente tras confirmar la cita.
- **IBAN**: `ES27 0182 5319 7002 0055 6013` · Titular: Carlos Antonio Acevedo · confirmado por Charly. (Ya no vive en el repo — estaba en el desaparecido `pagos.js`.)
- **Notificaciones internas**: ntfy.sh topic `perrosdelaisla-citas-2026` (`NTFY_TOPIC` en [victoria.js](victoria.js):57) — confirmado por Charly en su móvil. Dos disparadores: `_notificarCarlos` (cita confirmada) y `_notificarRescate` (lead dejó datos en s9 sin confirmar).
...
- `index.html` → carga `victoria.js?v=66`
- `victoria.js` → importa `agenda.js?v=66`, `supabase.js?v=66`, etc.
